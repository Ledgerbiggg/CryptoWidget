---
name: push-changes
description: 推送本次功能改动：先拉取（pull）再推送（push）本地提交。只做同步，不升版本、不改更新说明、不触发发版工作流。使用场景：用户说"/push-changes"或"推送改动"、"同步到远端"、"提交推送"时。
---

# 推送改动（Push Changes）

当前工作目录是 CryptoWidget 仓库根。本指令只负责把本地改动**拉取并推送**到远端，不升级版本号、不触发 GitHub 发版工作流。相比 /publish-release，它少了「升版本号 + 写更新清单」那一步。
