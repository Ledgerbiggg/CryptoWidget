...
description: 发布新版本：升级版本号 + 整理上次发版到当前的全部提交 + 写入 version.json 更新清单 + 提交并推送到远端。push 后 GitHub 工作流会自动据此生成 Release 说明。使用场景：用户说"/publish-release"或"发布版本"、"发版"、"升版本"、"出个版本"时。
---

# 发布新版本（Publish Release）

当前工作目录是 CryptoWidget 仓库根。请按顺序执行下列**完整工作流**完成一次版本发布。本指令只做版本发布与文档整理，**不要在代码里新增任何功能或文件**。

## 参数

- 用户输入的可选参数：`patch` / `minor` / `major`，默认 `patch`（第三位 +1，满 10 进位）。
...
（提交信息保持此英文格式，便于第 3 步的 `--grep` 能再次定位到本次为「上一次发版」）
